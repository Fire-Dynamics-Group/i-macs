...
def _get_all_sections() -> OrderedDict:
    ...
    # 3. Remaining Data.xml sections (non-UB families, or UB sections
    #    already in Data.xml that aren't in Blue Book)
...
def api_sections():
    """All sections grouped by family (includes custom sections)."""
...
def api_submit_run(request_body: dict):
    ...
    # Map form field deck_id -> DeckId (same as generate_combinations)
    if "deck_id" in params:
        params["DeckId"] = params["deck_id"]
...
def page_config(request: Request):
    ...
    # Check if any custom entries exist (for divider rendering)
    db = _get_db()
    custom_sections = db.get_custom_sections()
    custom_decks = db.get_custom_decks()
    custom_meshes = db.get_custom_meshes()
    db.close()
