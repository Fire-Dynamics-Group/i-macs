...
test.describe("FRC import", () => {
  test("header button → sample.frc populates form + shows import banner", async ({
    ...
    // Every form field gets a "●" — source-based provenance — so the
    // count tracks the FormValues field count (~45). Just assert there's
    // at least one per visible label and that the banner legend mentions
    // them.
    const dots = page.getByTestId("frc-imported-dot");
    expect(await dots.count()).toBeGreaterThan(20);
    await expect(banner).toContainText("still set to the imported value");
